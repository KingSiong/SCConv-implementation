...
#     def __init__(self, in_channels: int, out_channels: int, kernel_size: int = 3, stride: int = 1, padding: int = 1, n_groups: int = 16, gate_treshold: float = 0.5, alpha: float = 1 / 2, squeeze_ratio: int = 2, groups: int = 2):
#         super().__init__()

#         self.SRU = SRU(in_channels, n_groups=n_groups, gate_treshold=gate_treshold) 
#         self.CRU = CRU(in_channels=in_channels, out_channels=out_channels, kernel_size=kernel_size, alpha=alpha, squeeze_radio=squeeze_ratio, groups=groups)
...
if __name__ == '__main__':
    ...
    x = torch.randn(16, 96, 224, 224)
    # x2 = torch.randn(16, 96, 224, 224) 
    conv2d_model = nn.Sequential(nn.Conv2d(96, 64, kernel_size=2))
    flops1, params1 = profile(conv2d_model, (x, ))
    scconv_model = SCConv(96, 64, kernel_size=3, alpha=0.5, squeeze_ratio=2)  # out_channels > in_channels * (1-alpha) / squeeze_radio
    flops2, params2 = profile(scconv_model, (x, ))
    y1 = conv2d_model(x)
    y2 = scconv_model(x)
    print(y1.shape)
    print(y2.shape)
